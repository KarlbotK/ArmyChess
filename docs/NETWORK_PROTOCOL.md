...
`resumeToken` 原文只保存在当前浏览器会话中，不出现在公开日志、分享链接、房间广播或服务端持久化文件里。服务端只落盘 SHA-256 摘要，用恒定时间比较验证重连凭证。
...
这里故意没有交战棋子、胜负棋子和被消灭棋子的身份。`path` 只包含已经公开发生的一步所经过的坐标，用于落子后的短暂路线回放；它不会携带棋子类型。
...
玩家前四次超时产生 `TURN_TIMED_OUT` 并自动跳过；累计第 5 次时不再发送普通跳过事件，而是移除该方全部棋子并发送原因是 `TIMEOUT` 的 `PLAYER_ELIMINATED`。客户端固定表述为“某方累计超时 5 次，全军覆没”。
...
- 创建、加入、提交布局、移动、投降、超时和再战投票成功后，服务端同步保存房间快照；
- 快照包含座位、棋盘、公开状态、当前回合、修订号、超时次数、回合截止时间和再战票，不包含 WebSocket 连接、聊天记录或恢复凭证明文；
- 每个房间使用独立 JSON 文件并以临时文件原子替换，单个损坏文件不会阻止其他房间加载；
- 默认目录为 `server/data/rooms`，可用 `ARMYCHESS_DATA_DIR` 覆盖；Docker Compose 将其挂载到命名数据卷。
